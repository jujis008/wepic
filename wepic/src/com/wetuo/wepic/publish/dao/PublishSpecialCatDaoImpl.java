...
public Pager list(final String username,final int pageSize,final int pageNo)  {	
	...
	public List<PublishSpecialCat> specialCatuserOfStory(int uid) {
		...
		List<PublishSpecialCat> publishSpecialCats=
			this.getHibernateTemplate().find("from PublishSpecialCat special where special.pid="+storyid+" and special.user.id="+uid);
