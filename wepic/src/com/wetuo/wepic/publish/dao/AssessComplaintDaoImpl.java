...
public class AssessComplaintDaoImpl extends XgHibernateCommonDao implements AssessComplaintDao {
	private static Log log = LogFactory.getLog(AssessComplaintDaoImpl.class);
	private static int rowCount = 0;
	//////////////common crud//////////////////////////////////////////////
	public List<AssessComplaint> findAll(){ 
		return (List<AssessComplaint>)getHibernateTemplate().find("from AssessComplaint");
	}
	
	public List<AssessComplaint> findPart(String strFields, String[] arrStrValues){ 
		return (List<AssessComplaint>)getHibernateTemplate().find("from AssessComplaint where "+strFields, arrStrValues);
	}
...
public Pager list(final int pageSize,final int pageNo)  {	
	...
	List list = getHibernateTemplate().executeFind(new HibernateCallback()	{
		...
		public Object doInHibernate(Session session) throws HibernateException, SQLException {
			...
			Criteria criteria = session.createCriteria(AssessComplaint.class);
			AssessComplaintDaoImpl.rowCount = ((Integer) criteria.setProjection( Projections.rowCount()).uniqueResult()).intValue();
	...
	pager = new Pager(pageSize, pageNo, AssessComplaintDaoImpl.rowCount, list);
	return pager;
...
public Pager list(final String username,final int pageSize,final int pageNo)  {	
	...
	public Pager list(final Map<String,Object> mapSqlParam,final int pageSize,final int pageNo)  {
		...
		//???HibernateCallback?????????????
			public Object doInHibernate(Session session) throws HibernateException, SQLException {
				...
				//???Hibernate??????
				int intSerial = 0;
				Query q = session.createQuery("select count(*) from  AssessComplaint  a where " + strWhereParam );
				...
				AssessComplaintDaoImpl.rowCount = Integer.parseInt(count.toString());
				//
				intSerial = 0;
				q = session.createQuery("select a from AssessComplaint a where " + strWhereParam );
	...
	public boolean delete(Integer id)  {
		/*
		AssessComplaint record = select(id);
		Session session = null;
		Transaction tx = null;
		boolean b = true;
		try {
		...
		return b;
		*/
		Object obj = getHibernateTemplate().load(AssessComplaint.class,new Integer(id));
	...
	public AssessComplaint select(Integer id)  {
		/*
		Session session = null;
		AssessComplaint record = null;
		try {
			session = HibernateSessionFactory.currentSession();
			Query query = session.createQuery("from AssessComplaint where id=?");
			query.setInteger(0, id);
			record = (AssessComplaint) query.uniqueResult();
			query = null;
		...
		*/
		...
		return (AssessComplaint) getHibernateTemplate().get(AssessComplaint.class, new Integer(id));
