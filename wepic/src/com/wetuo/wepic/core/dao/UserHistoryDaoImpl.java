...
public class UserHistoryDaoImpl extends XgHibernateCommonDao implements UserHistoryDao {
	private static Log log = LogFactory.getLog(UserHistoryDaoImpl.class);
	private static int rowCount = 0;
	//////////////common crud//////////////////////////////////////////////
	@SuppressWarnings("unchecked")
	public List<UserHistory> findAll(){ 
		return (List<UserHistory>)getHibernateTemplate().find("from UserHistory");
	}
	
	public List<UserHistory> findPart(String strFields, String[] arrStrValues){ 
		return (List<UserHistory>)getHibernateTemplate().find("from UserHistory where "+strFields, arrStrValues);
	}
...
public Pager list(final int pageSize,final int pageNo)  {	
	...
	List list = getHibernateTemplate().executeFind(new HibernateCallback()	{
		...
		public Object doInHibernate(Session session) throws HibernateException, SQLException {
			...
			UserHistoryDaoImpl.rowCount = ((Integer) criteria.setProjection( Projections.rowCount()).uniqueResult()).intValue();
	...
	pager = new Pager(pageSize, pageNo, UserHistoryDaoImpl.rowCount, list);
	return pager;
...
public Pager list(final String username,final int pageSize,final int pageNo)  {	
	...
	public Pager list(final Map<String,Object> mapSqlParam,final int pageSize,final int pageNo)  {
		...
		//???HibernateCallback?????????????
			public Object doInHibernate(Session session) throws HibernateException, SQLException {
				...
				//???Hibernate??????
				int intSerial = 0;
				Query q = session.createQuery("select count(*) from  UserHistory  a where " + strWhereParam );
				...
				UserHistoryDaoImpl.rowCount = Integer.parseInt(count.toString());
				//
				intSerial = 0;
				q = session.createQuery("select a from UserHistory a where " + strWhereParam );
	...
	public boolean delete(Integer id)  {
		...
		return b;
		*/
		Object obj = getHibernateTemplate().load(UserHistory.class,new Integer(id));
	...
	public UserHistory select(Integer id)  {
		...
		try {
			session = HibernateSessionFactory.currentSession();
			Query query = session.createQuery("from UserHistory where id=?");
			query.setInteger(0, id);
			record = (UserHistory) query.uniqueResult();
			query = null;
		...
		*/
		...
		return (UserHistory) getHibernateTemplate().get(UserHistory.class, new Integer(id));
