...
public class BuyCartDealDaoImpl extends XgHibernateCommonDao implements BuyCartDealDao {
	private static Log log = LogFactory.getLog(BuyCartDealDaoImpl.class);
	private static int rowCount = 0;
	//////////////common crud//////////////////////////////////////////////
	public List<BuyCartDeal> findAll(){ 
		return (List<BuyCartDeal>)getHibernateTemplate().find("from BuyCartDeal");
	}
	
	public List<BuyCartDeal> findPart(String strFields, String[] arrStrValues){ 
		return (List<BuyCartDeal>)getHibernateTemplate().find("from BuyCartDeal where "+strFields, arrStrValues);
	}
...
public Pager list(final int pageSize,final int pageNo)  {	
	...
	List list = getHibernateTemplate().executeFind(new HibernateCallback()	{
		...
		public Object doInHibernate(Session session) throws HibernateException, SQLException {
			...
			Criteria criteria = session.createCriteria(BuyCartDeal.class);
			BuyCartDealDaoImpl.rowCount = ((Integer) criteria.setProjection( Projections.rowCount()).uniqueResult()).intValue();
	...
	pager = new Pager(pageSize, pageNo, BuyCartDealDaoImpl.rowCount, list);
	return pager;
...
public Pager list(final String username,final int pageSize,final int pageNo)  {	
	...
	public Pager list(final Map<String,Object> mapSqlParam,final int pageSize,final int pageNo)  {
		...
		//???HibernateCallback?????????????
			public Object doInHibernate(Session session) throws HibernateException, SQLException {
				...
				//???Hibernate??????
				int intSerial = 0;
				Query q = session.createQuery("select count(*) from  BuyCartDeal  a where " + strWhereParam );
				...
				BuyCartDealDaoImpl.rowCount = Integer.parseInt(count.toString());
				//
				intSerial = 0;
				q = session.createQuery("select a from BuyCartDeal a where " + strWhereParam );
	...
	public boolean delete(String id)  {
		...
		return b;
		*/
		Object obj = getHibernateTemplate().load(BuyCartDeal.class,new String(id));
	...
	public BuyCartDeal select(String id)  {
		/*
		Session session = null;
		BuyCartDeal record = null;
		try {
			session = HibernateSessionFactory.currentSession();
			Query query = session.createQuery("from BuyCartDeal where id=?");
			query.setInteger(0, id);
			record = (BuyCartDeal) query.uniqueResult();
			query = null;
		...
		*/
		...
		return (BuyCartDeal) getHibernateTemplate().get(BuyCartDeal.class, new String(id));
