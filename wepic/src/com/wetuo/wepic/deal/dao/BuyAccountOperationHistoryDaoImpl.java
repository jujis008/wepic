...
public class BuyAccountOperationHistoryDaoImpl extends XgHibernateCommonDao implements BuyAccountOperationHistoryDao {
	private static Log log = LogFactory.getLog(BuyAccountOperationHistoryDaoImpl.class);
	private static int rowCount = 0;
	//////////////common crud//////////////////////////////////////////////
	public List<BuyAccountOperationHistory> findAll(){ 
		return (List<BuyAccountOperationHistory>)getHibernateTemplate().find("from BuyAccountOperationHistory");
	}
	
	public List<BuyAccountOperationHistory> findPart(String strFields, String[] arrStrValues){ 
		return (List<BuyAccountOperationHistory>)getHibernateTemplate().find("from BuyAccountOperationHistory where "+strFields, arrStrValues);
	}
...
public Pager list(final int pageSize,final int pageNo)  {	
	...
	List list = getHibernateTemplate().executeFind(new HibernateCallback()	{
		...
		public Object doInHibernate(Session session) throws HibernateException, SQLException {
			...
			Criteria criteria = session.createCriteria(BuyAccountOperationHistory.class);
			BuyAccountOperationHistoryDaoImpl.rowCount = ((Integer) criteria.setProjection( Projections.rowCount()).uniqueResult()).intValue();
	...
	pager = new Pager(pageSize, pageNo, BuyAccountOperationHistoryDaoImpl.rowCount, list);
	return pager;
...
public Pager list(final String username,final int pageSize,final int pageNo)  {	
	...
	public Pager list(final Map<String,Object> mapSqlParam,final int pageSize,final int pageNo)  {
		...
		//???HibernateCallback?????????????
			public Object doInHibernate(Session session) throws HibernateException, SQLException {
				...
				//???Hibernate??????
				int intSerial = 0;
				Query q = session.createQuery("select count(*) from  BuyAccountOperationHistory  a where " + strWhereParam );
				...
				BuyAccountOperationHistoryDaoImpl.rowCount = Integer.parseInt(count.toString());
				//
				intSerial = 0;
				q = session.createQuery("select a from BuyAccountOperationHistory a where " + strWhereParam );
	...
	public boolean delete(String id)  {
		/*
		BuyAccountOperationHistory record = select(id);
		Session session = null;
		Transaction tx = null;
		boolean b = true;
		try {
		...
		return b;
		*/
		Object obj = getHibernateTemplate().load(BuyAccountOperationHistory.class,new String(id));
	...
	public BuyAccountOperationHistory select(String id)  {
		/*
		Session session = null;
		BuyAccountOperationHistory record = null;
		try {
			session = HibernateSessionFactory.currentSession();
			Query query = session.createQuery("from BuyAccountOperationHistory where id=?");
			query.setInteger(0, id);
			record = (BuyAccountOperationHistory) query.uniqueResult();
			query = null;
		...
		*/
		...
		return (BuyAccountOperationHistory) getHibernateTemplate().get(BuyAccountOperationHistory.class, new String(id));
