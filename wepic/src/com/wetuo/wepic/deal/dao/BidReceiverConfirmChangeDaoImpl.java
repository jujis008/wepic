...
public class BidReceiverConfirmChangeDaoImpl extends XgHibernateCommonDao implements BidReceiverConfirmChangeDao {
	private static Log log = LogFactory.getLog(BidReceiverConfirmChangeDaoImpl.class);
	private static int rowCount = 0;
	//////////////common crud//////////////////////////////////////////////
	public List<BidReceiverConfirmChange> findAll(){ 
		return (List<BidReceiverConfirmChange>)getHibernateTemplate().find("from BidReceiverConfirmChange");
	}
	
	public List<BidReceiverConfirmChange> findPart(String strFields, String[] arrStrValues){ 
		return (List<BidReceiverConfirmChange>)getHibernateTemplate().find("from BidReceiverConfirmChange where "+strFields, arrStrValues);
	}
...
public Pager list(final int pageSize,final int pageNo)  {	
	...
	List list = getHibernateTemplate().executeFind(new HibernateCallback()	{
		...
		public Object doInHibernate(Session session) throws HibernateException, SQLException {
			...
			Criteria criteria = session.createCriteria(BidReceiverConfirmChange.class);
			BidReceiverConfirmChangeDaoImpl.rowCount = ((Integer) criteria.setProjection( Projections.rowCount()).uniqueResult()).intValue();
	...
	pager = new Pager(pageSize, pageNo, BidReceiverConfirmChangeDaoImpl.rowCount, list);
	return pager;
...
public Pager list(final String username,final int pageSize,final int pageNo)  {	
	...
	public Pager list(final Map<String,Object> mapSqlParam,final int pageSize,final int pageNo)  {
		...
		//???HibernateCallback?????????????
			public Object doInHibernate(Session session) throws HibernateException, SQLException {
				...
				//???Hibernate??????
				int intSerial = 0;
				Query q = session.createQuery("select count(*) from  BidReceiverConfirmChange  a where " + strWhereParam );
				...
				BidReceiverConfirmChangeDaoImpl.rowCount = Integer.parseInt(count.toString());
				//
				intSerial = 0;
				q = session.createQuery("select a from BidReceiverConfirmChange a where " + strWhereParam );
	...
	public boolean delete(String id)  {
		/*
		BidReceiverConfirmChange record = select(id);
		Session session = null;
		Transaction tx = null;
		boolean b = true;
		try {
		...
		return b;
		*/
		Object obj = getHibernateTemplate().load(BidReceiverConfirmChange.class,new String(id));
	...
	public BidReceiverConfirmChange select(String id)  {
		/*
		Session session = null;
		BidReceiverConfirmChange record = null;
		try {
			session = HibernateSessionFactory.currentSession();
			Query query = session.createQuery("from BidReceiverConfirmChange where id=?");
			query.setInteger(0, id);
			record = (BidReceiverConfirmChange) query.uniqueResult();
			query = null;
		...
		*/
		...
		return (BidReceiverConfirmChange) getHibernateTemplate().get(BidReceiverConfirmChange.class, new String(id));
