...
public class BidDetailDaoImpl extends XgHibernateCommonDao implements BidDetailDao {
	private static Log log = LogFactory.getLog(BidDetailDaoImpl.class);
	private static int rowCount = 0;
	//////////////common crud//////////////////////////////////////////////
	public List<BidDetail> findAll(){ 
		return (List<BidDetail>)getHibernateTemplate().find("from BidDetail");
	}
	
	public List<BidDetail> findPart(String strFields, String[] arrStrValues){ 
		return (List<BidDetail>)getHibernateTemplate().find("from BidDetail where "+strFields, arrStrValues);
	}
...
public Pager list(final int pageSize,final int pageNo)  {	
	...
	List list = getHibernateTemplate().executeFind(new HibernateCallback()	{
		...
		public Object doInHibernate(Session session) throws HibernateException, SQLException {
			...
			Criteria criteria = session.createCriteria(BidDetail.class);
			BidDetailDaoImpl.rowCount = ((Integer) criteria.setProjection( Projections.rowCount()).uniqueResult()).intValue();
	...
	pager = new Pager(pageSize, pageNo, BidDetailDaoImpl.rowCount, list);
	return pager;
...
public Pager list(final String username,final int pageSize,final int pageNo)  {	
	...
	public Pager list(final Map<String,Object> mapSqlParam,final int pageSize,final int pageNo)  {
		...
		//???HibernateCallback?????????????
			public Object doInHibernate(Session session) throws HibernateException, SQLException {
				...
				//???Hibernate??????
				int intSerial = 0;
				Query q = session.createQuery("select count(*) from  BidDetail  a where " + strWhereParam );
				...
				BidDetailDaoImpl.rowCount = Integer.parseInt(count.toString());
				//
				intSerial = 0;
				q = session.createQuery("select a from BidDetail a where " + strWhereParam );
	...
	public boolean delete(String id)  {
		...
		return b;
		*/
		Object obj = getHibernateTemplate().load(BidDetail.class,new String(id));
	...
	public BidDetail select(String id)  {
		/*
		Session session = null;
		BidDetail record = null;
		try {
			session = HibernateSessionFactory.currentSession();
			Query query = session.createQuery("from BidDetail where id=?");
			query.setInteger(0, id);
			record = (BidDetail) query.uniqueResult();
			query = null;
		...
		*/
		...
		return (BidDetail) getHibernateTemplate().get(BidDetail.class, new String(id));
