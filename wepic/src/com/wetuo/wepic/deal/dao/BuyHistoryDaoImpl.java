...
public class BuyHistoryDaoImpl extends XgHibernateCommonDao implements BuyHistoryDao {
	private static Log log = LogFactory.getLog(BuyHistoryDaoImpl.class);
	private static int rowCount = 0;
	//////////////common crud//////////////////////////////////////////////
	public List<BuyHistory> findAll(){ 
		return (List<BuyHistory>)getHibernateTemplate().find("from BuyHistory");
	}
	
	public List<BuyHistory> findPart(String strFields, String[] arrStrValues){ 
		return (List<BuyHistory>)getHibernateTemplate().find("from BuyHistory where "+strFields, arrStrValues);
	}
...
public Pager list(final int pageSize,final int pageNo)  {	
	...
	List list = getHibernateTemplate().executeFind(new HibernateCallback()	{
		...
		public Object doInHibernate(Session session) throws HibernateException, SQLException {
			...
			Criteria criteria = session.createCriteria(BuyHistory.class);
			BuyHistoryDaoImpl.rowCount = ((Integer) criteria.setProjection( Projections.rowCount()).uniqueResult()).intValue();
	...
	pager = new Pager(pageSize, pageNo, BuyHistoryDaoImpl.rowCount, list);
	return pager;
...
public Pager list(final String username,final int pageSize,final int pageNo)  {	
	...
	public Pager list(final Map<String,Object> mapSqlParam,final int pageSize,final int pageNo)  {
		...
		//???HibernateCallback?????????????
			public Object doInHibernate(Session session) throws HibernateException, SQLException {
				...
				//???Hibernate??????
				int intSerial = 0;
				Query q = session.createQuery("select count(*) from  BuyHistory  a where " + strWhereParam );
				...
				BuyHistoryDaoImpl.rowCount = Integer.parseInt(count.toString());
				//
				intSerial = 0;
				q = session.createQuery("select a from BuyHistory a where " + strWhereParam );
	...
	public boolean delete(String id)  {
		...
		return b;
		*/
		Object obj = getHibernateTemplate().load(BuyHistory.class,new String(id));
	...
	public BuyHistory select(String id)  {
		/*
		Session session = null;
		BuyHistory record = null;
		try {
			session = HibernateSessionFactory.currentSession();
			Query query = session.createQuery("from BuyHistory where id=?");
			query.setInteger(0, id);
			record = (BuyHistory) query.uniqueResult();
			query = null;
		...
		*/
		...
		return (BuyHistory) getHibernateTemplate().get(BuyHistory.class, new String(id));
