function getArea(province, city, district) {
	this.dat = {
		"中国": {
			"北京q市": ["北京市1"],
	...
	this.text = [province, city, district];
	...
	this.cDat = [];
	this.dDat = [];
	this.init(province, city, district);
}
getArea.prototype = {
	init: function(p, c, d) {
		if (typeof this.dat[p] === 'object') {
			this.pName = p;
			for (var i in this.dat[p]) this.cDat.push(i);
			if (typeof this.dat[p][c] === 'object') {
				this.cName = c;
				this.dName = d;
				for (var i = 0,
				j; j = this.dat[p][c][i++];) this.dDat.push(j);
			}
		}
		for (var i in this.dat) this.pDat.push(i);
	},
