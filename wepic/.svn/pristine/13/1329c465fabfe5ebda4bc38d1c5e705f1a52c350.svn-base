...
<title>wepic管理_配置中心_账户安全_基本安全信息_修改邮箱_手机方式</title>
...
window.onload = function(){
	...
	$("#nextOnePasswordPhone").attr("disabled","disabled");
	}
	function validateemailcode(){
	     var writecode=$("#nextOnePasswordPhone").val();
	if(writecode == null || writecode == ""){
	   $("#passRemind").html("请输入邮件验证码");
	   $("#passRemind").css("color","#FF0000");
	}else{
	   $.ajax({
	      type:"Post",
		  url: "configurajson_changephonebyemailvalidate.do",
		  data:"emailcode="+writecode,
		  success : function(data) {
          var statu=data.state;
              if(statu == 1){
                   location.href="configure!changephonesteptwo.do";
              }else{
                   $("#passRemind").html("邮件验证码输入错误");
			       $("#passRemind").css("color","#FF0000");
			        location.href="configure!changephonefail.do";
              }        
		  }
	  });
	}
...
<style>
.container_body .info_detail{
	border:0;
	}
.container_body .info_detail .info_password {
    height: 260px;
    left: 0;
    margin: 80px auto;
    position: relative;
...
.container_body .info_detail .info_password table{
	width:500px;
    margin:0px auto;
	}
.container_body .info_detail .info_password table .tr_remind{
	text-align:center;
	height:50px;
	display:none;
	line-height:16px;
...
.container_body .info_detail .info_password table .td_one{
	text-align:right;
	width:200px;
	}
...
.container_body .info_detail .info_password .password_three {
    margin-left: 225px;
	}

.container_body .info_detail .info_password .password_four {
    margin-left: 275px;
	}
</style>
</head>
        ...
        <div class="info_detail">
        	<div class="div_step">
            	<div class="step_character">
            		<div class="step_now_character">邮箱验证</div>
            		<div class="step_gey_character">新手机验证</div>
            		<div class="step_gey_character">完成</div>
                </div>
            	<div class="step_ico">
            ...
            <div class="info_password">
            	<table>
                	<tr class="tr_phone">
                    	<td class="td_one"><span class="span_password">当前邮箱</span></td>
                    	<!-- 
                    	1****2@qq.com
                    	
                    	
                    	 -->
                    	 <s:set value="user.email" var="email" scope="request"></s:set>
                    	 <%
                    	 String email=request.getAttribute("email").toString();
                    	 String tempfir=email.substring(0,1);
                    	 int position=email.lastIndexOf("@");
                    	 String a=email.substring(0,position);
                    	 String templast=email.substring(a.length()-1,a.length());
                    	 String tempabc=email.substring(a.length(),email.length());
                    	  %>
                    	<td class="span_phone"><span><%=tempfir %>****<%=templast %><%=tempabc %></span>
                    	<input id="newemail" type="hidden" value="<s:property value="user.email"/>"/>
                    	</td>
                    </tr>
                	
                    <tr class="tr_remind" id="trRemind">
                    	<td colspan="2">我们已发送了一封邮件到您的当前邮箱，请将邮件中的验证码填入下方输入框，如未收到邮件，请查收其他邮件文件夹，如：垃圾邮件、广告邮件，验证有效期为8小时。</td>
                    </tr>
                    <tr class="tr_phone">
                    	<td class="td_one"><input type="button" value="前往查收" class="achBut" id="goToView" onclick="searchemail()" /></td>
                    	<td align="left"><input type="button" value="获取邮件验证码" class="achBut" id="achieveBut" onclick="achieveMobileMail('nextOnePasswordPhone','achieveBut');"/></td>
                    </tr>
                	<tr class="tr_coad">
                    	<td class="td_one"><span class="span_password">邮箱验证码</span></td>
                    	<td><input class="input_mobile" type="text" id="nextOnePasswordPhone"  disabled="disabled" /></td>
                    </tr>
                	<tr class="tr_a">
                    	<td></td>
                    	<td>
                        	<a href="<%=basePath %>configure!modifymobile.do">使用登录密码验证</a>
                        	<label id="passRemind"></label>
                        </td>
                    </tr>
                </table>
                ...
                <div class="password_three">
                    <input type="button" value="下一步"  class="login_submit" onclick="validateemailcode()"/>
                </div>
                <div class="password_four">
                    <a href="<%=basePath %>configure!securityIndex.do">取消操作</a>
                </div>
            </div>
        </div>
