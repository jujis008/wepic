...
public class PublishCat_PublishDaoImpl extends XgHibernateCommonDao implements PublishCat_PublishDao {
	private static Log log = LogFactory.getLog(PublishCat_PublishDaoImpl.class);
	private static int rowCount = 0;
	//////////////common crud//////////////////////////////////////////////
	@SuppressWarnings("unchecked")
	public List<PublishCat_Publish> findAll(){ 
		return (List<PublishCat_Publish>)getHibernateTemplate().find("from PublishCat_Publish");
	}
	
	public List<PublishCat_Publish> findPart(String strFields, String[] arrStrValues){ 
		return (List<PublishCat_Publish>)getHibernateTemplate().find("from PublishCat_Publish where "+strFields, arrStrValues);
	}
...
public Pager list(final int pageSize,final int pageNo)  {	
	...
	List list = getHibernateTemplate().executeFind(new HibernateCallback()	{
		...
		public Object doInHibernate(Session session) throws HibernateException, SQLException {
			...
			Criteria criteria = session.createCriteria(PublishCat_Publish.class);
			PublishCat_PublishDaoImpl.rowCount = ((Integer) criteria.setProjection( Projections.rowCount()).uniqueResult()).intValue();
	...
	pager = new Pager(pageSize, pageNo, PublishCat_PublishDaoImpl.rowCount, list);
	return pager;
...
public List list(final int publicId)  {	
	//Pager pager = null;	
	//??????HibernateCallback????4??��??
	...
	List list = getHibernateTemplate().executeFind(new HibernateCallback()	{
		...
		public Object doInHibernate(Session session) throws HibernateException, SQLException {
			//???Hibernate??????
			//List result = session.createQuery(hql).setParameter(0, value).setFirstResult(offset).setMaxResults(pageSize).list();
			Query criteria = session.createQuery("  from PublishCat_Publish as cat  where cat.publish.id="+publicId);
		List<?> result = criteria.list();
	...
	public Pager list(final Map<String,Object> mapSqlParam,final int pageSize,final int pageNo)  {
		...
		//???HibernateCallback?????????????
			public Object doInHibernate(Session session) throws HibernateException, SQLException {
				...
				//???Hibernate??????
				int intSerial = 0;
				Query q = session.createQuery("select count(*) from  PublishCat_Publish  a where " + strWhereParam );
				...
				PublishCat_PublishDaoImpl.rowCount = Integer.parseInt(count.toString());
				//
				intSerial = 0;
				q = session.createQuery("select a from PublishCat_Publish a where " + strWhereParam );
	...
	public boolean delete(Integer id)  {
		...
		return b;
		*/
		Object obj = getHibernateTemplate().load(PublishCat_Publish.class,new Integer(id));
...
//	}
	...
	public PublishCat_Publish select(Integer id)  {
		...
		*/
		...
		return (PublishCat_Publish) getHibernateTemplate().get(PublishCat_Publish.class, new Integer(id));
	...
	public Pager findByDetachedCriteria(DetachedCriteria detachedCriteria,int pageSize, int pageNo){
		
          
	     		
		
		List  list=super.getHibernateTemplate().findByCriteria(detachedCriteria,(pageNo-1)*pageSize,pageSize);
		
		int rowCount=((Integer) this.getHibernateTemplate().findByCriteria(detachedCriteria.setProjection(Projections.rowCount()),0,1).get(0)).intValue();  
