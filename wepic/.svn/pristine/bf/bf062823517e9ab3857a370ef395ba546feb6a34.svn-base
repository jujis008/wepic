...
public class BuyAccountOperationDaoImpl extends XgHibernateCommonDao implements BuyAccountOperationDao {
	private static Log log = LogFactory.getLog(BuyAccountOperationDaoImpl.class);
	private static int rowCount = 0;
	//////////////common crud//////////////////////////////////////////////
	public List<BuyAccountOperation> findAll(){ 
		return (List<BuyAccountOperation>)getHibernateTemplate().find("from BuyAccountOperation");
	}
	
	public List<BuyAccountOperation> findPart(String strFields, String[] arrStrValues){ 
		return (List<BuyAccountOperation>)getHibernateTemplate().find("from BuyAccountOperation where "+strFields, arrStrValues);
	}
...
public Pager list(final int pageSize,final int pageNo)  {	
	...
	List list = getHibernateTemplate().executeFind(new HibernateCallback()	{
		...
		public Object doInHibernate(Session session) throws HibernateException, SQLException {
			...
			Criteria criteria = session.createCriteria(BuyAccountOperation.class);
			BuyAccountOperationDaoImpl.rowCount = ((Integer) criteria.setProjection( Projections.rowCount()).uniqueResult()).intValue();
	...
	pager = new Pager(pageSize, pageNo, BuyAccountOperationDaoImpl.rowCount, list);
	return pager;
...
public Pager list(final String username,final int pageSize,final int pageNo)  {	
	...
	public Pager list(final Map<String,Object> mapSqlParam,final int pageSize,final int pageNo)  {
		...
		//???HibernateCallback?????????????
			public Object doInHibernate(Session session) throws HibernateException, SQLException {
				...
				//???Hibernate??????
				int intSerial = 0;
				Query q = session.createQuery("select count(*) from  BuyAccountOperation  a where " + strWhereParam );
				...
				BuyAccountOperationDaoImpl.rowCount = Integer.parseInt(count.toString());
				//
				intSerial = 0;
				q = session.createQuery("select a from BuyAccountOperation a where " + strWhereParam );
	...
	public boolean delete(String id)  {
		/*
		BuyAccountOperation record = select(id);
		Session session = null;
		Transaction tx = null;
		boolean b = true;
		try {
		...
		return b;
		*/
		Object obj = getHibernateTemplate().load(BuyAccountOperation.class,new String(id));
	...
	public BuyAccountOperation select(String id)  {
		/*
		Session session = null;
		BuyAccountOperation record = null;
		try {
			session = HibernateSessionFactory.currentSession();
			Query query = session.createQuery("from BuyAccountOperation where id=?");
			query.setInteger(0, id);
			record = (BuyAccountOperation) query.uniqueResult();
			query = null;
		...
		*/
		...
		return (BuyAccountOperation) getHibernateTemplate().get(BuyAccountOperation.class, new String(id));
	...
	public List findById(int accountId) {
		// TODO Auto-generated method stub
		Account account=this.getAccountService().select(accountId);
		DetachedCriteria dc = DetachedCriteria.forClass(BuyAccountOperation.class);
		dc.add(Restrictions.eq("operationType",4));
	...
	public List sellFindById(int accountId) {
		...
		Account account=this.getAccountService().select(accountId);
		DetachedCriteria dc = DetachedCriteria.forClass(BuyAccountOperation.class);
		dc.add(Restrictions.eq("operationType",3));
		dc.add(Restrictions.eq("account",account));
		List<BuyAccountOperation> list=this.getHibernateTemplate().findByCriteria(dc);
		return list;
	}
