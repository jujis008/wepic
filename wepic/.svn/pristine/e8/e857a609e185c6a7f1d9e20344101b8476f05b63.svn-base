...
public class UserCreditDaoImpl extends XgHibernateCommonDao implements UserCreditDao {
	private static Log log = LogFactory.getLog(UserCreditDaoImpl.class);
	private static int rowCount = 0;
	//////////////common crud//////////////////////////////////////////////
	public List<UserCredit> findAll(){ 
		return (List<UserCredit>)getHibernateTemplate().find("from UserCredit");
	}
	
	public List<UserCredit> findPart(String strFields, String[] arrStrValues){ 
		return (List<UserCredit>)getHibernateTemplate().find("from UserCredit where "+strFields, arrStrValues);
	}
...
public Pager list(final int pageSize,final int pageNo)  {	
	...
	List list = getHibernateTemplate().executeFind(new HibernateCallback()	{
		...
		public Object doInHibernate(Session session) throws HibernateException, SQLException {
			...
			Criteria criteria = session.createCriteria(UserCredit.class);
			UserCreditDaoImpl.rowCount = ((Integer) criteria.setProjection( Projections.rowCount()).uniqueResult()).intValue();
	...
	pager = new Pager(pageSize, pageNo, UserCreditDaoImpl.rowCount, list);
	return pager;
...
public Pager list(final String username,final int pageSize,final int pageNo)  {	
	...
	public Pager list(final Map<String,Object> mapSqlParam,final int pageSize,final int pageNo)  {
		...
		//???HibernateCallback?????????????
			public Object doInHibernate(Session session) throws HibernateException, SQLException {
				...
				//???Hibernate??????
				int intSerial = 0;
				Query q = session.createQuery("select count(*) from  UserCredit  a where " + strWhereParam );
				...
				UserCreditDaoImpl.rowCount = Integer.parseInt(count.toString());
				//
				intSerial = 0;
				q = session.createQuery("select a from UserCredit a where " + strWhereParam );
	...
	public boolean delete(Integer id)  {
		...
		return b;
		*/
		Object obj = getHibernateTemplate().load(UserCredit.class,new Integer(id));
	...
	public UserCredit select(Integer id)  {
		/*
		Session session = null;
		UserCredit record = null;
		try {
			session = HibernateSessionFactory.currentSession();
			Query query = session.createQuery("from UserCredit where id=?");
			query.setInteger(0, id);
			record = (UserCredit) query.uniqueResult();
			query = null;
		...
		*/
		...
		return (UserCredit) getHibernateTemplate().get(UserCredit.class, new Integer(id));
	...
	public UserCredit selectByUser(User user)  {
		...
		*/
		try{
			return (UserCredit) getHibernateTemplate().find("from UserCredit where user="+user.getId()).get(0);
		}catch (Exception e) {
			return null;
		}
