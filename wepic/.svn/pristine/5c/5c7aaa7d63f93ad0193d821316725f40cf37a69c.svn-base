...
public class SiteConfigDaoImpl extends XgHibernateCommonDao implements SiteConfigDao {
	private static Log log = LogFactory.getLog(SiteConfigDaoImpl.class);
	private static int rowCount = 0;
	//////////////common crud//////////////////////////////////////////////
	@SuppressWarnings("unchecked")
	public List<SiteConfig> findAll(){ 
		return (List<SiteConfig>)getHibernateTemplate().find("from SiteConfig");
	}
	
	@SuppressWarnings("unchecked")
	public List<SiteConfig> findPart(String strFields, String[] arrStrValues){ 
		return (List<SiteConfig>)getHibernateTemplate().find("from SiteConfig where "+strFields, arrStrValues);
	}
...
public Pager list(final int pageSize,final int pageNo)  {	
	...
	List list = getHibernateTemplate().executeFind(new HibernateCallback()	{
		...
		public Object doInHibernate(Session session) throws HibernateException, SQLException {
			...
			Criteria criteria = session.createCriteria(SiteConfig.class);
			SiteConfigDaoImpl.rowCount = ((Integer) criteria.setProjection( Projections.rowCount()).uniqueResult()).intValue();
	...
	pager = new Pager(pageSize, pageNo, SiteConfigDaoImpl.rowCount, list);
	return pager;
...
public Pager list(final String username,final int pageSize,final int pageNo)  {	
	...
	public Pager list(final Map<String,Object> mapSqlParam,final int pageSize,final int pageNo)  {
		...
		//???HibernateCallback?????????????
			public Object doInHibernate(Session session) throws HibernateException, SQLException {
				...
				//???Hibernate??????
				int intSerial = 0;
				Query q = session.createQuery("select count(*) from  SiteConfig  a where " + strWhereParam );
				...
				SiteConfigDaoImpl.rowCount = Integer.parseInt(count.toString());
				//
				intSerial = 0;
				q = session.createQuery("select a from SiteConfig a where " + strWhereParam );
	...
	public boolean delete(String id)  {
		...
		return b;
		*/
		Object obj = getHibernateTemplate().load(SiteConfig.class,new String(id));
	...
	public SiteConfig select(String id)  {
		/*
		Session session = null;
		SiteConfig record = null;
		try {
			session = HibernateSessionFactory.currentSession();
			Query query = session.createQuery("from SiteConfig where id=?");
			query.setInteger(0, id);
			record = (SiteConfig) query.uniqueResult();
			query = null;
		...
		*/
		...
		return (SiteConfig) getHibernateTemplate().get(SiteConfig.class, new String(id));
	...
	public SiteConfig getCurrentSiteConfig() {
		// TODO Auto-generated method stub
		return (SiteConfig)(getHibernateTemplate().find("from SiteConfig config order by config.configTime desc")).get(0);
	}
