...
<title>wepic管理_配置中心_账户安全_基本安全信息_修改邮箱_步骤二</title>
...
<script type="text/javascript">
	...
	var inputemail=$("#nextTwoPassword").val();
	if(inputemail == "" || inputemail == null){
		$("#passRemind").html("请输入新邮箱");
		$("#passRemind").css("color","#FF0000");
	}else{
	    var emailcheck = isEmail(inputemail);
			if(emailcheck==false){
				$("#passRemind").html("请输入正确的邮箱地址");
				$("#passRemind").css("color","#FF0000");
				}
			else{
				$.ajax({
    	      type:"Post",
    		  url: "configurajson_validatemail.do",
    		  data:"inputmail="+inputemail,
    		  success : function(data) {
              var statu=data.state;
              if(statu == 0){
              window.location = "configure!sendregemail.do";
              }else if(statu == 1){
                 $("#passRemind").html("该邮箱已注册");
		         $("#passRemind").css("color","#FF0000");
              }else{
                 $("#passRemind").html("邮件发送失败,请检查邮箱是否有效.");
                 $("#passRemind").css("color","#FF0000");
              }
        ...
        <div class="info_detail">
        	<div class="div_step">
            	<div class="step_character">
            		...
            		<div class="step_now_character">输入新邮箱</div>
            ...
            <div class="info_password">
            	<div class="password_one">
                    <span class="span_password">输入新邮箱</span>
                    <input class="input_email" type="text" value=""  id="nextTwoPassword" />
                    <label id="passRemind" style="width: 210px"></label>
                </div>
                
                <div class="password_two">
                    <span>此邮箱可作为登录账户，请选择常用邮箱，避免遗忘</span>
                </div>
                
                <div class="password_three">
                <input type="button" value="下一步"  class="login_submit" onclick="validatemail()"/>
<!--                    <input type="submit" value="下一步"  class="login_submit" onclick="nextTwo('nextTwoPassword','passRemind');"/>-->
                </div>
