...
$.ajax({
		...
		success : function(data) {
				...
				for(var i=0;i<datas.length;i++){
				    cxt+="<li><div class='img'><a href='javascript:;'><input type='hidden' value="+datas[i].id+"></input><img src='"+datas[i].thumbnail125+"' class='imgsrc' onload='javascript:DrawImage(this,125,125)'/></a></div><div class='nowtop'><div class='bj'>"+
...
function submitPopBox2(id){
	...
	if(addValue == "addSubStory"){
		...
		if($(this).attr("class")=="active"){
			...
			$.ajax({
			type : "POST",
			url : "jsonpublish_findthumbnailbyId2.do",
			data : "publishId="+id,				
			success : function(data) {
					var datas = data.resMsg;
					url2=datas;
					window.parent.closeOwnBox("chooseWork");
            		window.parent.fucropHead(url2,id);
					}
			}
		    )
			}
	...
	if(addValue == "addSubStory"){
		...
		if($(this).attr("class")=="active"){
			...
			success : function(data) {
					...
					url2=datas;
					window.parent.closeOwnBox("chooseWork");
            		window.parent.fucropHead(url2,tempid);
					}
			}
		    )
			}
...
function coverupload(){
 $("#storycoverupload").upload({
			...
			window.parent.closeOwnBox("chooseWork");
            window.parent.fucropHeadloading();
            },  
            onComplete: function (data, self, element) {  
				var data = eval("(" +data +")");
				if(data.flag==0){
				   alert("上传的图片至少一边大于或等于2000px");	
				   window.parent.removefucropHeadloading();		
				}else if(data.flag==2){
				  alert("仅支持上传jpg 和tif格式的图片");
				  window.parent.removefucropHeadloading();
				}else{
				   	 var cxt="<ul><li class='active'><div class='img'><a href='javascript:;'><img src="+data.msg+" class='imgsrc'/></a></div></li></ul>";
                     $("#divViewRe").html(cxt.toString());
                     submitPopBox2(data.publishId);
    	...
    	<div class="workAbstractRe">
				...
				<div class="editSelect" id="divSpecialType" >
                    	<div class="viewBelow" onclick="showBelow('divSelectPicValue')">
                        	<div id="selectPicValue">所有类别</div>
                ...
                <div class="editSelect" id="divWorkType" >
                    	<div class="viewBelow" onclick="showBelow('divSelectSpecialValue')">
                        	<div id="selectSpecialValue">所有专辑</div>
                       		<div class="viewBelowRight"><i></i></div>    
                    	</div>
