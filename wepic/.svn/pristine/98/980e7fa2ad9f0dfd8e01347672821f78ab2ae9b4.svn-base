...
public class ResLeftSideItemDaoImpl extends XgHibernateCommonDao implements ResLeftSideItemDao {
	private static Log log = LogFactory.getLog(ResLeftSideItemDaoImpl.class);
	private static int rowCount = 0;
	private ResMenuDao resMenuDao ;
	//////////////common crud/////////////////
	public List<ResLeftSideItem> findAll(){ 
		return (List<ResLeftSideItem>)getHibernateTemplate().find("from ResLeftSideItem");
	}
	
	public List<ResLeftSideItem> findByMenu(int menuId){ 
		//return (List<ResLeftSideItem>)getHibernateTemplate().find("from ResLeftSideItem");
		return getHibernateTemplate().find("From ResLeftSideItem as ResLeftSideItem where  ResLeftSideItem.type=1 and ResLeftSideItem.resMenu.id = "+ menuId + "  order by id desc");
	}
	
	public List<ResLeftSideItem> findPart(String strFields, String[] arrStrValues){ 
		return (List<ResLeftSideItem>)getHibernateTemplate().find("from ResLeftSideItem where "+strFields, arrStrValues);
	}
	...
	public Pager pagerByMenu( final int menuId, final int pageSize, final int pageNo ){
		...
		try{
			...
			String hql = "select count(*) from ResLeftSideItem as ResLeftSideItem where ResLeftSideItem.resMenu.id = " + menuId;
			Long countLong = (java.lang.Long) session.createQuery(hql).uniqueResult();
			int countInt = countLong.intValue();
			Query queryObject = session.createQuery("from ResLeftSideItem as ResLeftSideItem where ResLeftSideItem.resMenu.id = " + menuId);
		    queryObject.setFirstResult(pageSize * (pageNo - 1));
		    queryObject.setMaxResults(pageSize);
		    List<ResLeftSideItem> list = (List<ResLeftSideItem>)queryObject.list();
		...
		List list = getHibernateTemplate().executeFind(new HibernateCallback()	{
			public Object doInHibernate(Session session) throws HibernateException, SQLException {
				...
				resLeftSideItem.setResMenu(resMenuDao.select(menuId)); 
				Criteria criteria = session.createCriteria(ResLeftSideItem.class);
				criteria.add(Example.create(resLeftSideItem)); 
				ResLeftSideItemDaoImpl.rowCount = ((Integer) criteria.setProjection( Projections.rowCount()).uniqueResult()).intValue();
	...
	public Pager list(final int pageSize,final int pageNo)  {	
		...
		pager = new Pager(pageSize, pageNo, ResLeftSideItemDaoImpl.rowCount, list);
		return pager;
	}//end list()
	...
	public Pager listByMenu(final int menuId, final int pageSize,final int pageNo)  {	
		...
		List list = getHibernateTemplate().executeFind(new HibernateCallback()	{
			public Object doInHibernate(Session session) throws HibernateException, SQLException {
				...
				Criteria criteria = session.createCriteria(ResLeftSideItem.class);
				ResLeftSideItemDaoImpl.rowCount = ((Integer) criteria.setProjection( Projections.rowCount()).uniqueResult()).intValue();
				Example example = Example.create(resMenuDao.select(menuId)) ;
	...
	public Pager list(final Map<String,Object> mapSqlParam,final int pageSize,final int pageNo)  {
		...
		//???HibernateCallback?????????????
			public Object doInHibernate(Session session) throws HibernateException, SQLException {
				...
				//???Hibernate??????
				int intSerial = 0;
				Query q = session.createQuery("select count(*) from  ResLeftSideItem  a where " + strWhereParam );
				...
				ResLeftSideItemDaoImpl.rowCount = Integer.parseInt(count.toString());
				//
				intSerial = 0;
				q = session.createQuery("select a from ResLeftSideItem a where " + strWhereParam );
	...
	public boolean delete(Integer id)  {
		/*
		ResLeftSideItem record = select(id);
		Session session = null;
		Transaction tx = null;
		boolean b = true;
		try {
		...
		return b;
		*/
		Object obj = getHibernateTemplate().load(ResLeftSideItem.class,new Integer(id));
	...
	public ResLeftSideItem select(Integer id)  {
		/*
		Session session = null;
		ResLeftSideItem record = null;
		try {
			session = HibernateSessionFactory.currentSession();
			Query query = session.createQuery("from ResLeftSideItem where id=?");
			query.setInteger(0, id);
			record = (ResLeftSideItem) query.uniqueResult();
			query = null;
		...
		*/
		...
		return (ResLeftSideItem) getHibernateTemplate().get(ResLeftSideItem.class, new Integer(id));
