...
<title>wepic管理员_用户_实名认证待审核_个人_审核未通过</title>
...
$(function(){
        ...
        $.ajax({
  	      type:"Post",
  		  url: "configurajson_userprenext.do?usercatId="+id+"&userpersonOrCompany=1&userifIdentityTrue=3",
  		  data:"",		
...
<link type="text/css" rel="stylesheet" href="<%=basePath %>administrator/user/css/autonymAttestationCheckPresonalNoPass.css" />
       ...
       <input type="hidden" id="usercatid" value="<s:property value="usercatId"/>"/>
        <div class="div_job">
            <ul class="ul_job">
                ...
                <li onclick="adminpersonnopass()" class="active">全部职业</li>
                </s:if>
                <s:else>
            ...
            <div class="operation">
            	<input class="backToPrevious" type="button" value="返回列表" onclick="window.location='<%=basePath %>configure!adminpersonchecknopasslist.do'"/>
