...
public class BuyCommoditiesHistoryDaoImpl extends XgHibernateCommonDao implements BuyCommoditiesHistoryDao {
	private static Log log = LogFactory.getLog(BuyCommoditiesHistoryDaoImpl.class);
	private static int rowCount = 0;
	//////////////common crud//////////////////////////////////////////////
	public List<BuyCommoditiesHistory> findAll(){ 
		return (List<BuyCommoditiesHistory>)getHibernateTemplate().find("from BuyCommoditiesHistory");
	}
	
	public List<BuyCommoditiesHistory> findPart(String strFields, String[] arrStrValues){ 
		return (List<BuyCommoditiesHistory>)getHibernateTemplate().find("from BuyCommoditiesHistory where "+strFields, arrStrValues);
	}
...
public Pager list(final int pageSize,final int pageNo)  {	
	...
	List list = getHibernateTemplate().executeFind(new HibernateCallback()	{
		...
		public Object doInHibernate(Session session) throws HibernateException, SQLException {
			...
			Criteria criteria = session.createCriteria(BuyCommoditiesHistory.class);
			BuyCommoditiesHistoryDaoImpl.rowCount = ((Integer) criteria.setProjection( Projections.rowCount()).uniqueResult()).intValue();
	...
	pager = new Pager(pageSize, pageNo, BuyCommoditiesHistoryDaoImpl.rowCount, list);
	return pager;
...
public Pager list(final String username,final int pageSize,final int pageNo)  {	
	...
	public Pager list(final Map<String,Object> mapSqlParam,final int pageSize,final int pageNo)  {
		...
		//???HibernateCallback?????????????
			public Object doInHibernate(Session session) throws HibernateException, SQLException {
				...
				//???Hibernate??????
				int intSerial = 0;
				Query q = session.createQuery("select count(*) from  BuyCommoditiesHistory  a where " + strWhereParam );
				...
				BuyCommoditiesHistoryDaoImpl.rowCount = Integer.parseInt(count.toString());
				//
				intSerial = 0;
				q = session.createQuery("select a from BuyCommoditiesHistory a where " + strWhereParam );
	...
	public boolean delete(String id)  {
		/*
		BuyCommoditiesHistory record = select(id);
		Session session = null;
		Transaction tx = null;
		boolean b = true;
		try {
		...
		return b;
		*/
		Object obj = getHibernateTemplate().load(BuyCommoditiesHistory.class,new String(id));
	...
	public BuyCommoditiesHistory select(String id)  {
		/*
		Session session = null;
		BuyCommoditiesHistory record = null;
		try {
			session = HibernateSessionFactory.currentSession();
			Query query = session.createQuery("from BuyCommoditiesHistory where id=?");
			query.setInteger(0, id);
			record = (BuyCommoditiesHistory) query.uniqueResult();
			query = null;
		...
		*/
		...
		return (BuyCommoditiesHistory) getHibernateTemplate().get(BuyCommoditiesHistory.class, new String(id));
