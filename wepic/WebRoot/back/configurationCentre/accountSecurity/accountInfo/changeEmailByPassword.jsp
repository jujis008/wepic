...
<title>wepic管理_配置中心_账户安全_基本安全信息_修改邮箱_密码方式</title>
...
<script type="text/javascript">
    function usecurrentmobilevalidate(){
       var usermobile=$("#umobile").val();
       if(usermobile == null || usermobile ==""){
       alert("您尚未绑定手机");
       }else{
       location.href="configure!mobilevalidate.do";
       }    
    }
	function inputcurrentpwd(){
	var inputpwd=$("#nextOnePassword").val();
	if(inputpwd == "" || inputpwd == null){
		$("#passRemind").html("请输入当前密码");
		$("#passRemind").css("color","#FF0000");
	}else{
	var userid=$("#uid").val();
    $.ajax({
    	      type:"Post",
    		  url: "configurajson_validateloginpwd.do",
    		  data:"userId="+userid+"&inputpwd="+inputpwd,
    		  success : function(data) {
              var statu=data.state;
              if(statu == 0){
                 $("#passRemind").html("密码输入错误");
		         $("#passRemind").css("color","#FF0000");
		         $("#nextOnePassword").val("");
              }else{
                 window.location = "configure!changesteptwo.do";
              }
    ...
    <div class="container_body">
        ...
        <div class="info_detail">
            ...
            <div class="info_password">
            	...
            	<input type="hidden" id="uid" value="<s:property value='user.id'/>"/>
            	<input type="hidden" id="umobile" value="<s:property value='user.mobile'/>"/>
                    <span class="span_password">输入登录密码</span>
                    <input class="input_password" type="password" value="" id="nextOnePassword"/>
                    <label id="passRemind">输入您的位图网账户登录密码</label>
                </div>
                
                <div class="password_two">
                    <a style="cursor: pointer;" onclick="usecurrentmobilevalidate()">使用当前手机验证</a>
                </div>
                
                <div class="password_three">
                <input type="button" value="下一步"  class="login_submit" onclick="inputcurrentpwd()"/>
<!--                    <input type="submit" value="下一步"  class="login_submit" id="loginSubmit" onclick="nextOne('nextOnePassword','passRemind');"/>-->
                </div>
