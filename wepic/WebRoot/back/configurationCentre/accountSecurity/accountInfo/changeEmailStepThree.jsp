...
<title>wepic管理_配置中心_账户安全_基本安全信息_修改邮箱_步骤三</title>
<link type="text/css" rel="stylesheet" href="<%=basePath %>back/configurationCentre/accountSecurity/css/changeEmailByPasswordOne.css" />
...
<script type="text/javascript" src="<%=basePath %>back/configurationCentre/accountSecurity/js/changeEmailByPassword.js"></script>
...
function nextThree(code,remind){
	...
	function validatecode(){
	    var code=$("#nextThreeCode").val();
	    $("#passRemind").html("");
	    if(code == null || code == ""){
	       $("#passRemind").html("请输入验证码");
		   $("#passRemind").css("color","#FF0000");
	    }else{
	        $.ajax({
    	      type:"Post",
    		  url: "configurajson_validatecode.do",
    		  data:"inputcode="+code,
    		  success : function(data) {
              var statu=data.state;
              if(statu == 0){
                 $("#passRemind").html("验证码输入错误");
		         $("#passRemind").css("color","#FF0000");
		         //$("#nextThreeCode").val("");
		         location.href="configure!checkmailfail.do";
              }else{
                 window.location = "configure!modifysuc.do";
              }
        ...
        <div class="info_detail">
        	<div class="div_step">
            	<div class="step_character">
            		...
            		<div class="step_gey_character">输入新邮箱</div>
            		<div class="step_now_character">输入邮箱验证码</div>
            ...
            <div class="info_password">
            	<div class="code_character">
                	我们已向您的新邮箱<span style="font-weight: bold; color:black;"><s:property value="user.email"/></span>发送了一封邮件，请前往查收，并将邮件中的验证码填入下方输入框，如未找到，请查收其他邮件文件夹，如：垃圾邮件、广告邮件，验证有效期为48小时
                 </div>
                 
                 <div class="code_character">
                 <input type="hidden" id="newemail" value="<s:property value="user.email"/>"/>
                 <a onclick="searchemail()" style="padding:1px 4px;margin-right:80px; height:25px;width:100px;cursor:pointer;border:1px solid #c3c3c3;font-family:'微软雅黑';color:#000;border-radius:4px;background:#dedede;">前往查收邮件</a>
                 </div>
            	<div class="password_one">
            	
                    <span class="span_code">验证码</span>
                    <input class="input_code" type="text" value=""  id="nextThreeCode"/>
                   	<a href="javascript:" class="ReAchieve" onclick="resendmail()">重新获取</a> <label id="passRemind"></label>
                </div>
                                
                <div class="password_three">
<!--                    <input type="submit" value="下一步"  class="login_submit" onclick="nextThree('nextThreeCode','passRemind');"/>-->
                    <input type="button" value="下一步"  class="login_submit" onclick="validatecode()"/>
                </div>
