var img = {
	 ...
	 init:function(){
		...
		for(var i=0;i<img.imagebg_li.length;i++){
			...
			if(i!=0){
				img.small_pic_html.push("<li id='"+img.imagebg_li[i].getAttribute("id")+"' onclick='img.play("+i+",this)'><img src='"+img.imagebg_li[i].getAttribute("data-sPic")+"' /></li>");
				img.imagebg.getElementsByTagName("li")[i].style.display ="none";
			}else{
				img.small_pic_html.push("<li id='"+img.imagebg_li[i].getAttribute("id")+"'  onclick='img.play("+i+",this)' class='currently'><img src='"+img.imagebg_li[i].getAttribute("data-sPic")+"' /></li>");
			}
	...
	play:function(i,obj){
		...
		//alert();
		picCenter();
		changePicInfo($(img.imagebg_li[i]).attr("id"));
	}
