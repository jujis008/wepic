var img = {
	 ...
	 init:function(){
		...
		for(var i=0;i<img.imagebg_li.length;i++){
			if(i!=0){
				img.small_pic_html.push("<li id='"+img.imagebg_li[i].getAttribute("id")+"' onclick='img.play("+i+",this)'>"+img.imagebg_li[i].innerHTML+"</li>");
				img.imagebg.getElementsByTagName("li")[i].style.display ="none";
			}else{
				img.small_pic_html.push("<li id='"+img.imagebg_li[i].getAttribute("id")+"' onclick='img.play("+i+",this)' class='currently'>"+img.imagebg_li[i].innerHTML+"</li>");
			}
		}
