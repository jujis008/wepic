$(function(){
	topCenter();					/* 头部展开动作*/
	leaveToStart("barMainUL");   //主导航背景底边框
	
	
	
	
	document.getElementById("searchTextMain").value="输入搜索内容";
	
	achieveBelowMain("searchSelectMain","searchValueMain");			/*获取下拉框值并隐藏	选择类型*/
	
	
	
	
	
	hoverAddClass(".serverShare");   //分享背景
	...
	$(child).each(function(index, element) {
		$(this).hover(
				function(){
					setTimeout(function(){ 
						if ($(element).hasClass('hover')){
							$(element).find(".unfold-list").stop(true,true).slideDown(100).animate({opacity:'1',},100);
							} 
						else{
							return;}
					},100);
				...
				function(){
					setTimeout(function(){ 
						if (!$(element).hasClass('hover')){
							$(element).find(".unfold-list").stop(true,true).slideUp(100).animate({opacity:'0',},100);
							} 
						else{
							return;}
					},100);
...
/* hover 动作	---淡入淡出*/
function hoverOpacity(divName){
	var allLi = $(divName).find("li");
	$(allLi).each(function(index, element){
			$(this).hover(
				function(){
					$(element).find(".behind-slide").animate({opacity:'1',},300);
					},
				function(){
					$(element).find(".behind-slide").animate({opacity:'0',},300);
					}
			);
    });	
}
...
/* hover 动作	---小头像*/
function hoverOpacityOnly(divName){
	$(divName).hover(
		function(){
			$(divName).find(".behind-slide").animate({opacity:'1',},300);
			},
		function(){
			$(divName).find(".behind-slide").animate({opacity:'0',},300);
		});
}
/* hover 动作	---小头像*/
...
// bolewDiv		下拉框div列表
// showValue		显示值
function achieveBelowMain(bolewDiv,showValue){
	...
	$(".type-choose").each(function() {
        $(this).hover(
		function(){
			$(this).find(".s-choose").show();
		},
		function(){
			$(this).find(".s-choose").hide();
